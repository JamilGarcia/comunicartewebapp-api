...
exports.verifyUser = async (req,res) => {
    try {
        ...
        if(Object.keys(req.body).length !== 0){
            const usuario = await db.query("SELECT password_usuario, puesto_empresa FROM main_users WHERE correo = $1 ;",[correo]);
            ...
            /*3. Si usuario existe, revisar si contraseña es la misma que en la 
                base de datos */
                // select (Pass = crypt('Hola2', Pass)) from User_j
            const passwordValida = await db.query("SELECT (password_usuario = crypt($1, password_usuario))as Resultado FROM main_users where correo = $2; ",[password_usuario,correo]);
            if(passwordValida){
